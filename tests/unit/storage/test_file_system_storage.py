"""Tests for KeyValueFileSystemStorage (key-value storage)."""
...
class TestKeyValueFileSystemStorageBasicOperations:
    """Test basic CRUD operations."""

    def test_save_and_get_record(self, tmp_path):
        """Test saving and retrieving a record."""
        storage = KeyValueFileSystemStorage(base_dir=str(tmp_path))
...
class TestKeyValueFileSystemStorageExistence:
...
class TestKeyValueFileSystemStorageUpsert:
...
class TestKeyValueFileSystemStorageMultipleCollections:
...
class TestKeyValueFileSystemStorageSingleton:
    ...
    def test_same_path_returns_same_instance(self, tmp_path):
        ...
        storage1 = KeyValueFileSystemStorage(base_dir=base_dir)
        storage2 = KeyValueFileSystemStorage(base_dir=base_dir)
    ...
    def test_different_paths_return_different_instances(self, tmp_path):
        ...
        storage1 = KeyValueFileSystemStorage(base_dir=path1)
        storage2 = KeyValueFileSystemStorage(base_dir=path2)
    ...
    def test_normalized_paths_return_same_instance(self, tmp_path):
        ...
        storage1 = KeyValueFileSystemStorage(base_dir=str(storage_dir))
        storage2 = KeyValueFileSystemStorage(base_dir=str(storage_dir.resolve()))

        assert storage1 is storage2


class TestKeyValueFileSystemStorageFactory:
    """Test factory integration."""

    def test_factory_create_key_value_storage(self, tmp_path):
        """Test creating KeyValueFileSystemStorage through factory."""
        storage = StorageFactory.create_key_value_storage(storage_type="filesystem", base_dir=str(tmp_path))

        assert isinstance(storage, KeyValueFileSystemStorage)
...
class TestKeyValueFileSystemStorageValidation:
    ...
    def test_invalid_collection_name_path_traversal(self, tmp_path):
        ...
        storage = KeyValueFileSystemStorage(base_dir=str(tmp_path))
        with pytest.raises(StorageValidationError, match="Invalid collection"):
...
class TestKeyValueFileSystemStorageAtomicWrites:
