...
class MilvusIndexManager:
    ...
    def _create_schema_fields(self, *, dimension_mapping: dict[str, int]) -> list[FieldSchema]:  # NOSONAR python:S3776
        """
        ...
        for vector_column, dimension in dimension_mapping.items():
            ...
            vector_field_name = self.feature_mappings.get(vector_column, vector_column)

            logger.info(
                f"Adding dense vector field: source_column='{vector_column}', "
                f"milvus_field='{vector_field_name}', dimension={dimension}"
            )
        ...
        content_field_name = self.feature_mappings.get(
    ...
    def create_collection(self, *, dimension_mapping: dict[str, int]) -> None:
        ...
        try:
            ...
            if self.add_sparse_vector:
                ...
                content_field = self.feature_mappings.get(
                    OperatorConstants.Columns.DOC_COLUMN_DEFAULT, OperatorConstants.VectorDB.DEFAULT_TEXT_FIELD_NAME
                )
                ...
                logger.info(
                    f"[MILVUS COLLECTION] Dense vector mode - creating collection with {len(dimension_mapping)} vector field(s)"
                )
                schema = CollectionSchema(
                    fields=fields,
                    description=f"Collection for {self.collection_name}",
            ...
            logger.info(f"[MILVUS COLLECTION] ✓ Collection '{self.collection_name}' created successfully")
            ...
            for vector_column, dimension in dimension_mapping.items():
                ...
                logger.info(
                    f"[MILVUS COLLECTION] Creating index on dense vector field '{vector_field_name}' "
                    f"(source: '{vector_column}', dimension: {dimension})"
                )
